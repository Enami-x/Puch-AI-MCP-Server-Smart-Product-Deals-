...
# --- UPGRADED: Root URL now provides a Bootstrap page with a pastel theme ---
@app.route('/')
def home():
    """Provides a helpful, Bootstrap-styled HTML documentation page with pastel colors."""
    # Using a multi-line string to build a modern HTML page with Bootstrap
    html_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .header-jumbotron {
                background-color: #fff9e6; /* Soft Cream */
            }
            .card {
                border: none;
            ...
            .badge.bg-primary {
                background-color: #d1c4e9 !important; /* Pastel Purple */
                color: #4527a0;
            }
            .badge.bg-danger {
                background-color: #f8bbd0 !important; /* Pastel Pink */
                color: #880e4f;
            }
            .badge.bg-secondary {
    ...
    <body>
        <div class="container my-5">
            ...
            <footer class="text-center text-muted mt-5">
                <p>Developed for the Puch AI Hackathon</p>
            </footer>
        </div>
    </body>
    </html>
...
def search_products():
    ...
    if not all([keyword, max_price_str, serpapi_key]):
        return jsonify({"error":
                        "Missing 'keyword', 'max_price', or API key"}), 400
    ...
    for item in data.get("shopping_results", []):
        try:
            ...
            if price == 0 or not (min_price <= price <= max_price): continue
            ...
            clean_results.append({
                ...
                item.get("thumbnail"),
                "value_score":
                rating / price if price != 0 else 0
            })
